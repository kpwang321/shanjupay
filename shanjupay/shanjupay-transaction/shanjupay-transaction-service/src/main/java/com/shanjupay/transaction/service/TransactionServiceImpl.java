...
public class TransactionServiceImpl implements TransactionService {
     ...
     * @param tradeNo 支付宝或微信的交易流水号(第三方支付系统的订单)
     * @param state             订单状态  交易状态支付状态,0-订单生成,1-支付中(目前未使用),2-支付成功,4-关闭 5--失败
     */
    @Override
    public void updateOrderTradeNoAndTradeState(String outTradeNo, String tradeNo, String state) throws BusinessException {
        LambdaUpdateWrapper<PayOrder> payOrderLambdaUpdateWrapper = new LambdaUpdateWrapper<>();
        payOrderLambdaUpdateWrapper.eq(PayOrder::getTradeNo,outTradeNo)
                .set(PayOrder::getTradeState,state)
                .set(PayOrder::getPayChannelTradeNo,tradeNo);
